    ...
    <>
      ...
      {toast && (
        <div className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-ink dark:bg-gray-700 text-white px-4 py-2.5 rounded-xl shadow-xl text-[13px] font-medium">
          ...
          <button onClick={() => { toast.onUndo(); dismissToast(); }} className="text-accentLight hover:text-white border border-accentLight/40 rounded-md px-2.5 py-1 text-xs">Undo</button>
          <button onClick={dismissToast} className="text-gray-400 hover:text-white ml-1 text-lg leading-none">✕</button>
        </div>
      )}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-5">
        <div className="flex items-center gap-3">
          <button onClick={() => setSelectedMonth(shiftMonthKey(selectedMonth, -1))} className="border border-line dark:border-gray-600 rounded-md w-8 h-8 text-ink dark:text-gray-200 hover:bg-paper dark:hover:bg-gray-700 transition-colors">‹</button>
        ...
        <Btn onClick={openAdd}><IconPlus /> Add entry</Btn>
      </div>

      {/* Stats - show both spend and income */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <StatCard label="Spent this month" value={<Amount>{fmtINR(totalSpend)}</Amount>} />
        <StatCard label="Income this month" value={<Amount><span className="text-emerald">{fmtINR(totalIncome)}</span></Amount>} />
        <StatCard label="Top spend category" value={topCat ? topCat[0] : '—'} delta={topCat ? fmtINR(topCat[1]) : ''} deltaClass="text-inkMuted dark:text-gray-400" />
        <StatCard label="vs last month" value={diff === 0 ? '—' : `${diff > 0 ? '▲' : '▼'} ${fmtINR(Math.abs(diff))}`} deltaClass={diff > 0 ? 'text-clay' : 'text-emerald'} />
      </div>

      {/* Chart */}
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        <div className="inline-flex border border-line dark:border-gray-600 rounded-lg overflow-hidden">
          {[['category','By Category'],['pie','Pie'],['daily','Daily'],['months','6-Month']].map(([key, label]) => (
            <button key={key} onClick={() => setPerspective(key)}
        ...
        <div className="flex items-center gap-2">
          ...
          <a href={exportUrl}><Btn variant="secondary"><IconDownload /> CSV</Btn></a>
        </div>
      </div>
      ...
      <Card className="mb-5">
        <div className="h-[220px]">
          {chartData.some(d => d.value > 0) ? (
            <ResponsiveContainer width="100%" height="100%">
              {perspective === 'category' ? (
                ...
                <PieChart>
                  <Pie data={chartData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} innerRadius={40} label={({ name, percent }) => `${name.slice(0,10)} ${(percent*100).toFixed(0)}%`} labelLine={false} fontSize={10}>
                    {chartData.map((_, i) => <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}
                  </Pie>
                  <Tooltip formatter={v => fmtINR(v)} contentStyle={{ fontSize: 12 }} />
                </PieChart>
          ...
          ) : <EmptyState title="Nothing to chart yet">Add an entry to see patterns.</EmptyState>}
        </div>
      </Card>
      ...
      <div className="flex items-center gap-2 flex-wrap mb-3">
        <div className="relative flex-1 min-w-[160px]">
          ...
          <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-inkMuted text-sm pointer-events-none">⌕</span>
        </div>
        ...
        <select className={`${inputClass} w-auto`} value={filterAccount} onChange={e => setFilterAccount(e.target.value)}>
      ...
      <SectionTitle>{monthLabel(selectedMonth)} — {filteredRows.length} entr{filteredRows.length === 1 ? 'y' : 'ies'}</SectionTitle>

      {filteredRows.length ? (
        <Card padded={false} className="overflow-x-auto">
          <table>
            <thead>
              <tr><th>Date</th><th>Type</th><th>Category</th><th>Description</th><th>Account</th><th>Notes</th><th className="num">Amount</th><th></th></tr>
            </thead>
            <tbody>
              {filteredRows.map(e => {
                ...
                return (
                  <tr key={e.id}>
                    ...
                    <td><Tag tone={TYPE_TAG_TONE[eType] || 'def'}>{EXPENSE_TYPE_LABELS[eType] || eType}</Tag></td>
                    <td><Tag>{e.category}</Tag></td>
                    <td className="text-ink dark:text-gray-200 max-w-[140px] text-[12.5px]">{e.description || <span className="text-inkMuted">—</span>}</td>
                    <td>{acct ? <span className="text-[10px] font-mono px-2 py-0.5 rounded-full whitespace-nowrap" style={{ background: acct.color + '22', color: acct.color }}>{acct.name}</span> : <span className="text-inkMuted dark:text-gray-600 text-xs">—</span>}</td>
                    <td className="text-inkMuted dark:text-gray-500 text-xs max-w-[100px]">{e.notes || '—'}</td>
                    ...
                    <td>
                      <div className="flex items-center gap-0.5">
                        <IconBtn danger={false} onClick={() => openEdit(e)} title="Edit"><IconEdit /></IconBtn>
                        <IconBtn danger={false} onClick={() => openClone(e)} title="Clone" className="text-[13px]">⧉</IconBtn>
                        <IconBtn onClick={() => handleDelete(e)} title="Delete"><IconTrash /></IconBtn>
                      </div>
                    </td>
                  </tr>
                );
        ...
        <EmptyState title={search || filterType !== 'all' ? 'No matches' : 'Nothing logged this month'}>
          {search ? `No entries match "${search}"` : filterType !== 'all' ? `No ${EXPENSE_TYPE_LABELS[filterType] || filterType} entries this month.` : `Add an entry to start tracking ${monthLabel(selectedMonth)}.`}
        </EmptyState>
      )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Edit entry' : 'Add entry'}>
        <div className="inline-flex border border-line dark:border-gray-600 rounded-lg overflow-hidden mb-4">
          {[['expense','Expense'],['income','Income']].map(([val, label]) => (
            <button key={val} onClick={() => setForm({ ...form, expenseType: val, category: val === 'income' ? INCOME_CATEGORIES[0] : EXPENSE_CATEGORIES[0] })}
        ...
        <Field label="Description">
          <input className={inputClass} placeholder={form.expenseType === 'income' ? 'e.g. June salary, freelance project' : 'e.g. Swiggy order, cab to office'} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
        </Field>
        <div className="grid grid-cols-2 gap-3 mt-3">
          ...
          <Field label="Account" hint={form.expenseType === 'income' ? 'Credits to balance.' : 'Deducts from balance.'}>
        ...
        <Field label="Notes (optional)" hint="Split with, reimbursable, occasion…">
          <input className={inputClass} placeholder="e.g. Split with Arjun · Client reimbursable" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} />
        </Field>
        <ModalActions>
