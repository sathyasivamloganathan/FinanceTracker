...
export function computeCategoryTotals(state) {
  ...
  // Fixed Deposits (active only) — principal value reflects in net worth
  const activeFDs = (state.fds || []).filter(fd => fd.status === 'active');
  if (activeFDs.length) {
    totals['Fixed Deposits'] = (totals['Fixed Deposits'] || 0) +
      activeFDs.reduce((s, fd) => s + (Number(fd.principal) || 0), 0);
  }
  // Bank account balances
  const bankTotal = (state.bankAccounts || []).reduce((s, a) => s + (Number(a.balance) || 0), 0);
  if (bankTotal) totals['Bank Accounts'] = (totals['Bank Accounts'] || 0) + bankTotal;
  return totals;
}
